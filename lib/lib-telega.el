...
(defvar +wd/ledger-file-name "~/Sync/org/ledger/current.ledger")
(defvar +wd/ledger-file-name "~/Sync/org/ledger/accounts")
...
(defun +wd/write-transactions (transaction-text file-name chat-date)
  ...
  (with-mutex ledger-mutex
    ...
    (when (string= (system-name) "arch-nuc") ;; only record on arch-nuc
      (with-temp-buffer
...
(defun +wd/creditcard-transaction (chat-text chat-date)
  (let* ((card-number-regexp "尾号\\(5048\\|6798\\|2972\\|6912\\)\\w*")
         ...
         (transaction-date-time nil) ; 后面可以补充上，如何从字符串中获取得到具体日期信息
